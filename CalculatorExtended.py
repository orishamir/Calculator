...
from SearchFuncs import searchChar, searchSplittingComma, splitSigma, searchSigma
...
def calc(expr: str):
    ...
    if idxAdd != -1:
        ...
        i = searchSplittingComma(expr, idxLog)
        a, b = expr[2:i], expr[i+1:-1]
        return _log10(calc(b)) / _log10(calc(a))

    elif idxSum != -1:
        var, start, end, func = searchSigma(expr)

        start = int(calc(start))
        end = int(calc(end))
        Sum = 0
...
expr = "S(a=1,5,S(b=a+1,6,a*S(c=1,a+b,a+b-c)))"  # "√(S(a=1,25,1))"  # "S(a=l(2,8),3,S(b=l(2,2^2),2,a*b+2))"  # "sin(√l(l(l(4,16),8),3^9)*10)"  # "6!/(5!*6+1)"
print("\n\n")
