...
def drawNode(x, y, node, spacingX, spacingY):
    ...
    pen.pensize(1)
    pen.write(
        str(toDraw).replace("/", "÷"), font=("Cascadia", fontSize, ""), align="center"
    )
    ...
    if type(node) not in [float, int, Num, Var]:
        ...
        pen.penup()
        drawNode(
            leftNodePos[0], leftNodePos[1], node.left, spacingX / 1.5, spacingY * 1.5
        )
        ...
        drawNode(
            rightNodePos[0], rightNodePos[1], node.right, spacingX / 1.5, spacingY * 1.5
        )
